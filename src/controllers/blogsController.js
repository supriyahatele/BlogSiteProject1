...
const blogs = async function (req, res) {
    try {
        if (Object.keys(data).length !== 0) {
            let data = req.body
            if (!data.author_Id) return res.send({ msgs: "author id not present" })

            let savedData = await authorModel.findById(data.author_Id)
            if (!savedData) return res.send("Invalid author id")

            let dataCreated = await blogsModel.create(data)
...
const deleteQuery  = async function(req,res){
    ...
    try {
     ...
     if (!validId(author_Id)) return res.status(400).send({ status: false, msg: "Invalid author id!" })
     let deletedData = await blogsModel.findOneAndUpdate({ category:category,author_Id:author_Id,tags:tags,subCategory:subCategory, isPublished: ispublished }, { isDeleted: true }, { new: true })
